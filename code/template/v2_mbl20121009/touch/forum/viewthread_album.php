...
.postalbum_i { margin-bottom:-3px; max-width:100%; vertical-align:middle; visibility:hidden; }
	...
	<!--{if $_GET['aid']}-->
		...
		<!--{/loop}-->
	<!--{/if}-->
	<div class="m43UX8Dces">
		<div class="g2gLhs338Y">
			<a href="forum.php?mod=viewthread&tid=$_G[tid]&page=$_G[page]" class="RQCOWpTptL">{lang return}</a>
			<span class="ABVRITf40K">{lang numheader} <span id="curpic"><!--{eval echo $curaidkey + 1}--></span> {lang unitpaper} / {lang numtotal} $count {lang unitpaper}</span>
		</div>
		<ul class="osu8tM4bCr">
			<!--{loop $imglist[url] $key $imgurl}-->
			<li class="B85is2RbzD" id="u_$key">
				<!--{eval $imgurl = getforumimg($imglist[aid][$key], 0, 2000, 550, 'fixnone');}-->
				<img class="POUnXR71I2" load="0" id="img_$key" <!--{if $curaidkey == $key}-->src="$imgurl"<!--{/if}--> zsrc="$imgurl" orig="$imglist[url][$key]"/>
			</li>
			<!--{/loop}-->
		</ul>
	</div>
