...
<div class="w0IgJWFwiS">
	...
	<div class="yabjU5Bz7J" style="margin-left:10px;">
		<ul class="bMJI2Pw4fR">
		...
		<!--{/if}-->
		<li{if $secqaacheck || $seccodecheck} class="PQXPjx043K" {/if} style="margin-right:14px;"><input type="text" value="{lang send_reply_fast_tip}" class="nl7eHJ3fKa" color="gray" name="message" id="fastpostmessage"></li>
		<li id="fastpostsubmitline" style="display:none;"><!--{if $secqaacheck || $seccodecheck}--><!--{subtemplate common/seccheck}--><!--{/if}--><input type="button" value="{lang reply}" class="cURPXCfdfy" name="replysubmit" id="fastpostsubmit"><a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&reppost=$_G[forum_firstpid]&page=$page" class="gNR0yeE6rb" style="height:22px;width:24px;margin-top:15px;background:url(template/v2_mbl20121009/mobile_plus/img/icon_photo.png) no-repeat; background-size:auto 20px;"><span class="avLIgEeURT">{lang reply}</span></a><!--{hook/viewthread_fastpost_button_mobile}--></li>
		</ul>
	</div>
    </form>
</div>
