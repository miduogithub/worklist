...
//在这每次读取worklist最新数据并同步到localStorage里
	...
	$.ajax({
	   type: "POST",
	   url: "work.php?mod=workcp&ac=notebook&action=getworklist", 
	   data:{}, 
	   dataType: "json",
	   success: function(data){
			...
			localStorage.setItem("notes-app",""+data.notesapp+"");

			var worklist_notesapplist = data.notesapplist;
			var worklist_notesapplistcount = data.notesappcount;
			


			for (var i = 0; i < worklist_notesapplistcount; i++) {
				
				var jsnotesappid = worklist_notesapplist[i].notesappid;
				var jsnotesappdata = worklist_notesapplist[i].notesappdata;

				localStorage.setItem(""+ jsnotesappid +"",jsnotesappdata );
...
//localStorage.setItem("notes-app","1,4");
//var idata = JSON.stringify({"id":4,"name":"miduo","description":"222222222222","date":1525790429648});

 //localStorage.setItem("notes-app-4",idata );
...
//alert(id);
	 ...
	 //console.log(e);
	  ...
	  //Ajax调用处理
		$.ajax({
		   type: "POST",
		   url: "work.php?mod=workcp&ac=notebook&action=close_xxx", 
		   data:{ 
				workid : id,
			}, 
		   dataType: "json",
  ...
  var NoteListView = Backbone.View.extend({
    ...
    create: function(e) {
	  ...
	  //console.log(note.id);
		...
		var newworkid = note.id;
	
		  //Ajax调用处理
		$.ajax({
		   type: "POST",
		   url: "work.php?mod=workcp&ac=notebook&action=create", 
		   data:{ 
				newworkid : newworkid,
			}, 
		   dataType: "json",
...
//console.log(id);
	...
	$.ajax({
	   type: "POST",
	   url: "work.php?mod=workcp&ac=notebook&action=updateOnKeyup", 
	   data:{ 
			workid : id,
			name : name,
			message : description,
