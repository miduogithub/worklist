...
foreach($openidlist as $openid) {
	...
	if($_G['timestamp'] > $today_morning_dateline_start && $_G['timestamp'] < $today_morning_dateline_end) {
		$time = '今天上午';
		$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_wx_checkin_logs')." WHERE editorid='$weixinmemberinfo_uid' AND dateline>'$today_morning_dateline_start' AND dateline<'$today_morning_dateline_end' AND checkaction='0'"), 0);
	}
	...
	if($_G['timestamp'] > $today_morning_dateline_start && $_G['timestamp'] < $today_afternoon_dateline_end) {
		$time = '今天下午';
		$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_wx_checkin_logs')." WHERE editorid='$weixinmemberinfo_uid' AND dateline>'$today_afternoon_dateline_start' AND dateline<'$today_afternoon_dateline_end' AND checkaction='0'"), 0);
	}
	...
	if($count == '0' && $week != '7') {//有新任务
		...
		if(!empty($weixinmemberinfo_uid) && $memberinfo['groupid'] == '1' || $memberinfo['groupid'] == '24') {
			...
			$message = ''.$memberinfo['username'].'，您'.$time.'该签到了';
			//考勤提醒
			$msgtype = '考勤提醒';
