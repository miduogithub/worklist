...
$query = DB::query("SELECT * FROM ".DB::table('work_financial_project')." WHERE projectid='$projectid'");	
...
if(empty($count)) {
	$work_financial_project_mission_Projectprice_count = DB::result(DB::query("SELECT sum(Projectprice) FROM ".DB::table('work_financial_project_mission')." $wheresql"), 0);

	$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_financial_project_mission')." $wheresql"), 0);
}
...
$query = DB::query("SELECT * FROM ".DB::table('work_financial_project_mission')." $wheresql ORDER BY status ASC,missionstarttimeline DESC $LIMIT");	
while ($value = DB::fetch($query)) {


	 $value['missionname'] = str_replace(array("\r\n", "\r", "\n"), "", $value['missionname']);   
/*
	$tasklistcount = '';
...
*/
	...
	if(!empty($value['work_project_projectname'])) {
		$missionlist['title'] = '【所属项目：'.$value['work_project_projectname'].'】- '.$value['missionname'];
	} else {
	...
	$missionlist['typename'] = $typelist['ttypelist'][$value['projecttypeid']]['typename'];
	...
	$missionlist['Projectprice'] = $value['Projectprice'];
...
$objPHPExcel->getActiveSheet()->setCellValue('A1', ''.$editor.'的「'.$missionlist_json['porjectinfo']['projectname'].'」报销明细');
...
foreach($missionlist as $list) {
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('F'.$i.'', ''.$list['Projectprice'].'');
		...
		if(!empty($list['tasklist'])) {
			...
			foreach($list['tasklist'] as $tlist) {
				...
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A'.$l.'', '')
							->setCellValue('B'.$l.'', '')
							->setCellValue('C'.$l.'', ''.$m.'、'.$tlist['taskname'].'')
							->setCellValue('D'.$l.'', '')
...
//加边框	  
$objPHPExcel->getActiveSheet()->getStyle('A'.$yuangongqianzi_hang.':F'.$yuangongqianzi_hang.'')->applyFromArray(
...
$objPHPExcel->getActiveSheet()->setCellValue('F'.$yuangongqianzi_hang.'', $work_financial_project_mission_Projectprice_count);
...
header('Content-Disposition: attachment;filename="'.$editor.'的「'.$missionlist_json['porjectinfo']['projectname'].'」报销明细_financial_project_uid_'.$editorid.'_dateline_'.$excelcode.'.xlsx"');
