...
<?php
		...
		showtableheader('');
		showsubtitle(array('', 'display_order', 'forums_admin_name', '', 'forums_moderators', '<a href="javascript:;" onclick="if(getmultiids()) location.href=\''.ADMINSCRIPT.'?action=work&operation=edit&multi=\' + getmultiids();return false;">'.$lang['multiedit'].'</a>'));
		...
		if(count($fids) != count($showed)) {
			foreach($fids as $fid) {
				if(!in_array($fid, $showed)) {
					C::t('work_forum_forumtest')->update($fid, array('fup' => '0', 'type' => 'forum'));
				}
			}
		}
		...
		if(is_array($_GET['order'])) {
			foreach($_GET['order'] as $fid => $value) {
				C::t('work_forum_forumtest')->update($fid, array('name' => $_GET['name'][$fid], 'displayorder' => $_GET['order'][$fid]));
			}
		}
		...
		if(is_array($_GET['newcat'])) {
			foreach($_GET['newcat'] as $key => $forumname) {
				...
				$fid = C::t('work_forum_forumtest')->insert(array('type' => 'group', 'name' => $forumname, 'status' => 1, 'displayorder' => $_GET['newcatorder'][$key]), 1);
				C::t('work_forum_forumtestfield')->insert(array('fid' => $fid));
			}
		}
		...
		if(is_array($_GET['newforum'])) {
			...
			foreach($_GET['newforum'] as $fup => $forums) {
				...
				$fupforum = C::t('work_forum_forumtest')->get_forum_by_fid($fup);

				if(empty($fupforum)) continue;

				if($fupforum['fup']) {
					$groupforum = C::t('work_forum_forumtest')->get_forum_by_fid($fupforum['fup']);
				} else {
					$groupforum = $fupforum;
				}
				...
				foreach($forums as $key => $forumname) {
					...
					if(!empty($inheritedid)) {
						...
						$forum = C::t('work_forum_forumtest')->get_forum_by_fid($inheritedid);
						$forumfield =  C::t('work_forum_forumtest')->get_forum_by_fid($inheritedid, null, 'forumfield');
					...
					$forumfields['fid'] = $fid = C::t('work_forum_forumtest')->insert($data, 1);
					...
					C::t('work_forum_forumtestfield')->insert($data);
		...
		cpmsg('forums_update_succeed', 'action=work', 'succeed');
	}
	...
	if(!submitcheck('modsubmit')) {
		...
		$forum = C::t('work_forum_forumtest')->fetch($fid);
		...
		if(!empty($_GET['delete']) || $_GET['newmoderator'] || (bool)$forum['inheritedmod'] != (bool)$inheritedmodnew) {
			...
			if($forum['type'] == 'group') {
				$query = C::t('work_forum_forumtest')->fetch_all_fids(1, 'forum', $fid);
				foreach($query as $sub) {
					$fidarray[] = $sub['fid'];
				}
				$query = C::t('work_forum_forumtest')->fetch_all_fids(1, 'sub', $fidarray);
				foreach($query as $sub) {
					$fidarray[] = $sub['fid'];
				}
			} elseif($forum['type'] == 'forum') {
				$query = C::t('work_forum_forumtest')->fetch_all_fids(1, 'sub', $fid);
				foreach($query as $sub) {
					$fidarray[] = $sub['fid'];
				}
			}
...
*/
			...
			C::t('work_forum_forumtest')->update($fid, array('inheritedmod' => $inheritedmodnew));
		}
		...
		foreach($fidarray as $fid) {
			...
			C::t('work_forum_forumtestfield')->update($fid, array('moderators' => $moderators));
		}
		cpmsg('forums_moderators_update_succeed', "mod=forum&action=work&operation=moderators&fid=$fid", 'succeed');
	...
	if(!submitcheck('mergesubmit') || $source == $target) {
		...
		if(C::t('work_forum_forumtest')->check_forum_exists(array($source,$target)) != 2) {
			cpmsg_error('forums_nonexistence');
		}
		if(C::t('work_forum_forumtest')->fetch_forum_num('', $source)) {
		...
		$sourceforum = C::t('work_forum_forumtest')->fetch_info_by_fid($source);
		$targetforum = C::t('work_forum_forumtest')->fetch_info_by_fid($target);
		...
		C::t('work_forum_forumtest')->update($target, array('threads' => $targetforum['threads'] + $sourceforum['threads'], 'posts' => $targetforum['posts'] + $sourceforum['posts']));
		C::t('work_forum_forumtestfield')->update($target, array('threadtypes' => serialize($targethreadtypes)));
		//C::t('forum_threadclass')->update_by_fid($source, array('fid' => $target));
		C::t('work_forum_forumtest')->delete_by_fid($source);
		...
		cpmsg('forums_merge_succeed', 'action=work', 'succeed');
	}
	...
	if(empty($fids)) {
		cpmsg('forums_edit_nonexistence', 'action=work&operation=edit'.(!empty($highlight) ? "&highlight=$highlight" : '').(!empty($anchor) ? "&anchor=$anchor" : ''), 'form', array(), '<select name="fid">'.forumselect(FALSE, 0, 0, TRUE).'</select>');
	}
	$mforum = array();
	$perms = array('viewperm', 'postperm', 'replyperm', 'getattachperm', 'postattachperm', 'postimageperm');

	$query = C::t('work_forum_forumtest')->fetch_all_info_by_fids($fids);
	if(empty($query)) {
	...
	if(!submitcheck('detailsubmit')) {
		...
		foreach($_G['cache']['forums'] as $forums) {
			...
			if($forums['type'] == 'group') {
				...
				$forumselect .= '</div><em class="cl">'.
					...
					'<span class="pointer" onclick="sdisplay(\'g_'.$forums['fid'].'\', this)"><img src="static/image/admincp/desc.gif" class="vmiddle" /></span> <span class="pointer" onclick="location.href=\''.ADMINSCRIPT.'?action=work&operation=edit&switch=yes&fid='.$forums['fid'].'\'">'.$forums['name'].'</span></em><div id="g_'.$forums['fid'].'" style="display:">';
			} elseif($forums['type'] == 'forum') {
				$forumselect .= '<input class="left checkbox ck" chkvalue="'.$sgid.'" name="multi[]" value="'.$forums['fid'].'" type="checkbox" '.($checked ? 'checked="checked" ' : '').'/><a class="f'.($checked ? ' current"' : '').'" href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=work&operation=edit&switch=yes&fid='.$forums['fid'].($mforum[0]['type'] != 'group' ? '&anchor=\'+currentAnchor' : '\'').'+\'&scrolltop=\'+scrollTopBody()">'.$forums['name'].'</a>';
			} elseif($forums['type'] == 'sub') {
				$forumselect .= '<input class="left checkbox ck" chkvalue="'.$sgid.'" name="multi[]" value="'.$forums['fid'].'" type="checkbox" '.($checked ? 'checked="checked" ' : '').'/><a class="s'.($checked ? ' current"' : '').'" href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=work&operation=edit&switch=yes&fid='.$forums['fid'].($mforum[0]['type'] != 'group' ? '&anchor=\'+currentAnchor' : '\'').'+\'&scrolltop=\'+scrollTopBody()">'.$forums['name'].'</a>';
			}
		}
		...
		showformheader("work&operation=edit&fid=$fid&", 'enctype');
		...
		if(count($mforum) == 1 && $mforum[0]['type'] == 'group') {
			...
			$selected_mv = $forum['modtype'] == 'mod+view' ? 'selected' : '';
			$selected_mm = $forum['modtype'] == 'mod+mod' ? 'selected' : '';

			showsetting('导航类型（modtype）', '', '', '
											<select name="modtypenew">
												<option value="mod+view" '.$selected_mv.'>mod+view</option>
												<option value="mod+mod" '.$selected_mm.'>mod+mod</option>
											</select>','','','mod+view	/ mod+mod');

			showsetting('模块名称（modcode）', 'modcodenew', dhtmlspecialchars($mforum[0]['modcode']), 'text');

			showsetting('ICON代码（iconcode）', 'iconcodenew', dhtmlspecialchars($mforum[0]['iconcode']), 'text');

			showsetting('forums_edit_basic_redirect', 'redirectnew', $forum['redirect'], 'text');

			showsetting('“新窗”是否默认勾选', 'targetblanknew', $forum['targetblank'], 'radio');

			showsetting('是否显示', 'statusnew', $mforum[0]['status'], 'radio');
			...
			if($multiset) {
				...
				foreach($mforum as $forum) {
					$_G['showtableheader_multi'][] = '<a href="javascript:;" onclick="location.href=\''.ADMINSCRIPT.'?action=work&operation=edit&fid='.$forum['fid'].'&anchor=\'+$(\'cpform\').anchor.value;return false">'.$forum['name'].'(fid:'.$forum['fid'].')</a>';
				}
			}
			$mfids = array();
			foreach($mforum as $forum) {
				...
				if(!$multiset) {
					...
					$query = C::t('work_forum_forumtest')->fetch_all_info_by_ignore_fid($fid);
...
EOT;
	...
	<?php
		...
		if($_GET['type'] == 'group') {
			if($_GET['namenew']) {
				...
				C::t('work_forum_forumtest')->update($fid, array(
					'name' => $_GET['namenew'],
					...
					'styleid' => $newstyleid,
					'modtype' => $_GET['modtypenew'],
					'modcode' => $_GET['modcodenew'],
					'iconcode' => $_GET['iconcodenew'],
					'targetblank' => $_GET['targetblanknew'],

				));

				$extranew = is_array($_GET['extranew']) ? $_GET['extranew'] : array();
				$extranew = serialize($extranew);
				C::t('work_forum_forumtestfield')->update($fid, array(
					'extra' => $extranew,
				...
				if($newstyleid != $mforum[0]['styleid'] && !empty($subfids)) {
					C::t('work_forum_forumtest')->update($subfids, array('styleid' => $newstyleid));
				}
				...
				cpmsg('forums_edit_succeed', 'action=work', 'succeed');
			} else {
			...
			if($_GET['fupnew'] != $forum['fup'] && !$multiset) {
				if(C::t('work_forum_forumtest')->fetch_forum_num('', $fid)) {
					cpmsg('forums_edit_sub_notnull', '', 'error');
				}

				$fup = C::t('work_forum_forumtest')->fetch($_GET['fupnew']);
			...
			C::t('work_forum_forumtest')->update($fid, $forumdata);

			if(!(C::t('work_forum_forumtestfield')->fetch($fid))) {
				C::t('work_forum_forumtestfield')->insert(array('fid' => $fid));
			}
			...
			if($forumfielddata) {
				C::t('work_forum_forumtestfield')->update($fid, $forumfielddata);
			}
			if($pluginsetting) {
		...
		cpmsg('forums_edit_succeed', "mod=forum&action=work&operation=edit&".($multiset ? 'multi='.implode(',', $_GET['multi']) : "fid=$fid").($_GET['anchor'] ? "&anchor={$_GET['anchor']}" : ''), 'succeed');
	...
	if($_GET['ajax']) {
		...
		if($currow + $pp > $total) {
			...
			C::t('work_forum_forumtest')->delete_by_fid($fid);
		...
		if($_GET['finished']) {
			updatecache('work');
			cpmsg('forums_delete_succeed', 'action=work', 'succeed');
		...
		if(!$_GET['confirmed']) {
			...
			cpmsg('forums_delete_confirm', "action=work&operation=delete&fid=$fid&formhash=".FORMHASH, 'form');

		} else {

			$threads = C::t('forum_thread')->count_by_fid($fid);
			cpmsg('forums_delete_alarm', "action=work&operation=delete&fid=$fid&confirmed=1&formhash=".FORMHASH, 'loadingform', '', '<div id="percent">0%</div>', FALSE);
			...
			<script type=\"text/JavaScript\">
				...
				function forumsdelete(url, total, pp, currow) {
					...
					x.get(url+'&ajax=1&pp='+pp+'&total='+total+'&currow='+currow, function(s) {
						if(s != 'GO') {
							location.href = adminfilename + '?action=work&operation=delete&finished=1&formhash=".FORMHASH."';
						}
				...
				forumsdelete(adminfilename + '?action=work&operation=delete&fid=$fid&confirmed=1&formhash=".FORMHASH."', $threads, 2000, 0);
			</script>
			";
		}
	}
	...
	$fields = array(
		'forums' 	=> C::t('work_forum_forumtest')->fetch_table_struct('work_forum_forumtest'),
		'forumfields'	=> C::t('work_forum_forumtest')->fetch_table_struct('work_forum_forumtestfield'),
	);
	...
	if(!submitcheck('copysubmit')) {
			...
			foreach($_GET['options'] as $option) {
				if($option = trim($option)) {
					if(in_array($option, $fields['forums'])) {
						$forumoptions['work_forum_forumtest'][] = $option;
					} elseif(in_array($option, $fields['forumfields'])) {
						$forumoptions['work_forum_forumtestfield'][] = $option;
					}
				}
			}
		}
		...
		foreach(array('work_forum_forumtest', 'work_forum_forumtestfield') as $table) {
		...
		updatecache('forums');
		cpmsg('forums_copy_succeed', 'action=work', 'succeed');
...
function showforum(&$forum, $type = '', $last = '', $toggle = false) {
	...
	if($last == '') {
		...
		$return .= '<input type="text" name="name['.$forum['fid'].']" value="'.dhtmlspecialchars($forum['name']).'" class="txt" />'.
			...
			<a href="'.ADMINSCRIPT.'?action=work&operation=edit&fid='.$forum['fid'].'" title="'.cplang('forums_edit_comment').'" class="act">'.cplang('edit').'</a>'.
			($type != 'group' ? '<a href="'.ADMINSCRIPT.'?action=work&operation=copy&source='.$forum['fid'].'" title="'.cplang('forums_copy_comment').'" class="act">'.cplang('forums_copy').'</a>' : '').
			'<a href="'.ADMINSCRIPT.'?action=work&operation=delete&fid='.$forum['fid'].'&formhash='.FORMHASH.'" title="'.cplang('forums_delete_comment').'" class="act">'.cplang('delete').'</a></td></tr>';
		...
		if($last == 'lastboard') {
			...
			$return = '</tbody><tr><td></td><td colspan="4"><div><a href="###" onclick="addrow(this, 0)" class="addtr">'.cplang('forums_admin_add_category').'</a></div></td>'.
				'<td class="bold"><a href="javascript:;" onclick="if(getmultiids()) location.href=\''.ADMINSCRIPT.'?action=work&operation=edit&multi=\' + getmultiids();return false;">'.cplang('multiedit').'</a></td>'.
				'</tr>';
		}
	}
...
function showforum_moderators($forum) {
	...
	if($forum['moderators']) {
		...
		if($count > 2) {
			...
			$r = '<a href="'.ADMINSCRIPT.'?action=work&operation=moderators&fid='.$forum['fid'].'" title="'.cplang('forums_moderators_comment').'">'.$r.' &raquo;</a>';
		...
		$r = '<a href="'.ADMINSCRIPT.'?action=work&operation=moderators&fid='.$forum['fid'].'" title="'.cplang('forums_moderators_comment').'">'.cplang('forums_admin_no_moderator').'</a>';
	}
	return $r;
}

function getthreadclasses_html($fid) {
	$threadtypes = C::t('work_forum_forumtestfield')->fetch($fid);
