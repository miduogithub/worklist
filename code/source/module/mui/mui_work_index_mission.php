...
$userlist_wx_checkin_logs = array();
...
//项目收入
	...
	if(!empty($editorid) && $_G['adminid'] != '1') {
	...
	while ($value = DB::fetch($query)) {
		...
		$missionquery = DB::query("SELECT * FROM ".DB::table('work_project_mission')." WHERE projectid='$value[projectid]' AND status='0' $wheresql2 ORDER BY status,missionstarttimeline ASC");
		while ($missionvalue = DB::fetch($missionquery)) {

			$wxuserinfo = get_wx_userinfo_by_uid($missionvalue['editorid']);
			
			if(!empty($wxuserinfo['headimgurl'])) {
				$missionvalue['avatar']  = $wxuserinfo['headimgurl'];
			} else {
				$missionvalue['avatar']  = avatar($missionvalue['editorid'], 'small', true);
			}
			//$value[missionlist][$value['projectid']] = $missionvalue; 
			$value['missionlist'][] = $missionvalue; 
		}

		$value['missioncount'] = count($value['missionlist']);
		...
		if($_G['adminid'] == '1') {
			...
			//只显示进行中的项目
			if($value['status'] == '0' && !empty($value['missionlist'])) {
			...
			//只显示结算中的项目
			if($value['status'] == '1' && !empty($value['missionlist'])) {
		...
		if(!empty($value['missionlist']) && $_G['adminid'] != '1') {
			//$threadlist[] = $value; 
		}
	}
...
//报销支出
	...
	while ($value = DB::fetch($query)) {
		...
		$missionquery = DB::query("SELECT * FROM ".DB::table('work_financial_project_mission')." WHERE projectid='$value[projectid]' $wheresql2 ORDER BY missionstarttimeline ASC");
		while ($missionvalue = DB::fetch($missionquery)) {
			//$value[missionlist][$value['projectid']] = $missionvalue; 
			$value['missionlist'][] = $missionvalue; 
...
$mui_app_index_list['Projectcountstatus0_threadlist'] = $Projectcountstatus0_threadlist;
$mui_app_index_list['Projectcountstatus0_threadlist_count'] = $Projectcountstatus0;
$mui_app_index_list['Projectcountstatus1_threadlist'] = $Projectcountstatus1_threadlist;
$mui_app_index_list['Projectcountstatus1_threadlist_count'] = $Projectcountstatus1;
$mui_app_index_list['Financial_threadlist'] = $Financial_threadlist;
$mui_app_index_list['Financial_Projectpricestatus0'] = $Financial_Projectpricestatus0;

$mui_app_index_list['userlist_wx_checkin_logs'] = $memberinfolist;
$mui_app_index_list['userlist_count'] = $membercount;


//echo json_encode($Projectcountstatus0_threadlist);
