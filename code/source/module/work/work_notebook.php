...
if($view == 'core') {
	...
	$query = DB::query("SELECT * FROM ".DB::table('work_loglist')." $wheresql ORDER BY $order dateline DESC LIMIT 100;");
	...
	include template('diy:work/pc/work_notebook_newadmin_main_core');
	exit;
}
...
if(checkMobile() == true && $_G['cookie']['mobile'] != 'no') {//$_G['cookie']['mobile'] 手机切换电脑版会种入cookie 所以增加判断条件
	...
	include template('diy:work/pc/work_file_notebook');
