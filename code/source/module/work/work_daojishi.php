...
if(empty($count)) {
	...
	//待办总数
	$count_status0 = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_daojishi')." WHERE dateline >= '$thedatelinestart' AND dateline < '$thedatelineend' AND status='0'"), 0);
}

$query = DB::query("SELECT * FROM ".DB::table('work_daojishi')." $wheresql ORDER BY $order dateline DESC");
...
$membermissioninfo = get_user_missionlist_by_uid($_G['uid']);

if(checkMobile() == true && $_G['cookie']['mobile'] != 'no') {//$_G['cookie']['mobile'] 手机切换电脑版会种入cookie 所以增加判断条件


	include template('diy:work/pc/work_daodishi_newadmin');
...
/*
function create_lastpostdateline_update_dateline_to_lastpostdateline() {
	$query = DB::query("SELECT * FROM ".DB::table('work_daojishi')." WHERE 1");
	while ($value = DB::fetch($query)) {

		if(empty($value['lastpostdateline'])) {
			$workid = $value['workid'];
			$newthreadinfo['lastpostdateline'] = $value['dateline'];
