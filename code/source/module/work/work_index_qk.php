...
//获取用户浏览项目的行为记录
$project_setting_list = get_work_setting_project_status_by_uid($_G['uid']);
...
//项目收入
	...
	while ($value = DB::fetch($query)) {
		...
		//项目进度老算法没有排除 相关资料 类型的任务
		//$value['progress'] = ($missioncount - $missioncount_status_0) / $missioncount * 100;

		//项目进度（排除了相关资料类型的任务）
		$value['progress'] = get_project_progress_by_projectid($value['projectid']);
	...
	//在此更新缓存
	require_once libfile('function/setcache');
	setcache_project_favicon_1($editorid,$Projectcountstatus0_threadlist_favicon_1,$Projectcountstatus0_favicon_1);
...
include template('diy:work/pc/work_index_qk');
