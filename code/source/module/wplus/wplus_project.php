...
if($view == 'addproject') {
	...
	$missioncount = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_project_mission')." WHERE projectid='$projectid'"), 0);
	$missioncount_status_0 = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_project_mission')." WHERE projectid='$projectid' AND status='0'"), 0);



	$project['progress'] = ($missioncount - $missioncount_status_0) / $missioncount * 100;
	...
	$missionlist = array();
	$missionquery = DB::query("SELECT * FROM ".DB::table('work_project_mission')." WHERE projectid='$projectid' ORDER BY status,missionstarttimeline DESC");
	while ($missionvalue = DB::fetch($missionquery)) {
		//$value[missionlist][$value['projectid']] = $missionvalue; 
		...
		if($missionvalue['projecttypeid'] == '26') {//26 为相关资料
			$informationlist[] = $missionvalue; 
		} else {
			$missionlist[] = $missionvalue; 
		}
	...
	include template('diy:wplus/pc/wplus_project_project_index');
	...
	if(empty($count)) {
		...
		$projectstatus9count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_project')."  $wheresql AND status='1'"), 0);
