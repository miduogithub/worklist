...
if(empty($count)) {
	...
	//当前商品库上传商品
	$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_shop_goodslog_admin')." $wheresql"), 0);

	//当前商品库上传商品合格数
	$status1count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_shop_goodslog_admin')." $wheresql AND status = '1'"), 0);

	//商品库总上传商品
	$goodslogcount = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_shop_goodslog_admin')." WHERE 1"), 0);

	//商品库总上传商品总合格数
	$goodslogstatus1count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('work_shop_goodslog_admin')." WHERE status = '1'"), 0);
...
$query = DB::query("SELECT * FROM ".DB::table('work_shop_goodslog_admin')." $wheresql ORDER BY editor,dateline ASC LIMIT 1000");
while ($value = DB::fetch($query)) {
	$threadlist[] = $value; 
}


include template('diy:work/pc/work_goodslist_newadmin');
