...
class table_work_forum_forumtest extends discuz_table
{
	public function __construct() {

		$this->_table = 'work_forum_forumtest';
		$this->_pk    = 'fid';
	...
	public function fetch_info_by_fid($fid) {
		return DB::fetch_first("SELECT ff.*, f.* FROM %t f LEFT JOIN %t ff ON ff.fid=f.fid WHERE f.fid=%d", array($this->_table, 'work_forum_forumtestfield', $fid));
	}
	...
	public function fetch_all_info_by_fids($fids, $status = 0, $limit = 0, $fup = 0, $displayorder = 0, $onlyforum = 0, $noredirect = 0, $type = '', $start = 0) {
		...
		return DB::fetch_all("SELECT ff.*, f.* FROM %t f LEFT JOIN %t ff USING (fid) WHERE $sql $ordersql $limitsql", array($this->_table, 'work_forum_forumtestfield'), 'fid');
	}
	public function fetch_all_default_recommend($num = 10) {
		return DB::fetch_all("SELECT f.fid, f.name, ff.description, ff.icon FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff USING(fid) WHERE f.status='3' AND f.type='sub' ORDER BY f.commoncredits desc ".DB::limit($num));
	}
	public function fetch_all_group_type($alltypeorder = 0) {
		$ordersql = empty($alltypeorder) ? 'f.type, ' : "f.type<>'group', ";
		return DB::fetch_all("SELECT f.fid, f.type, f.status, f.name, f.fup, f.displayorder, f.forumcolumns, f.inheritedmod, ff.moderators, ff.password, ff.redirect, ff.groupnum FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff USING(fid) WHERE f.status='3' AND f.type IN('group', 'forum') ORDER BY $ordersql f.displayorder");
	}
	public function fetch_all_recommend_by_fid($fid) {
		return DB::fetch_all("SELECT ff.*, f.* FROM %t f LEFT JOIN %t ff ON ff.fid=f.fid WHERE f.recommend=%d", array($this->_table, 'work_forum_forumtestfield', $fid));
	}
	...
	public function fetch_all_forum($status = 0) {
		...
		return DB::fetch_all("SELECT ff.*, f.*, a.uid FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff ON ff.fid=f.fid LEFT JOIN ".DB::table('forum_access')." a ON a.fid=f.fid AND a.allowview>'0' WHERE $statusql ORDER BY f.type, f.displayorder");
	}
	...
	public function fetch_all_forum_ignore_access() {
		return DB::fetch_all("SELECT ff.*, f.* FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff ON ff.fid=f.fid WHERE status <3 ORDER BY f.fid");
	}
	public function fetch_all_forum_for_sub_order() {
		return DB::fetch_all("SELECT ff.*, f.fid, f.type, f.status, f.name, f.fup, f.displayorder, f.inheritedmod, f.domain, f.modcode, f.iconcode, f.modtype, f.targetblank FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff USING(fid) WHERE f.status<>'3' ORDER BY f.type<>'group', f.displayorder");
	}
	...
	public function fetch_all_valid_fieldinfo() {
		return DB::fetch_all("SELECT ff.* FROM ".DB::table($this->_table)." f INNER JOIN ".DB::table('work_forum_forumtestfield')." ff USING(fid) WHERE f.status='1'");
	}
	...
	public function fetch_all_for_threadsorts() {
		return DB::fetch_all("SELECT f.fid, f.name, ff.threadsorts FROM ".DB::table($this->_table)." f , ".DB::table('work_forum_forumtestfield')." ff WHERE ff.threadsorts<>'' AND f.fid=ff.fid");
	}
	...
	public function fetch_all_for_search($conditions, $start = 0, $limit = 20) {
		...
		if($start == -1) {
			return DB::result_first("SELECT count(*) FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff ON f.fid=ff.fid
			WHERE status='3' AND type='sub' AND %i", array($conditions));
		}
		return DB::fetch_all("SELECT f.fid, f.fup, f.type, f.name, f.posts, f.threads, ff.membernum, ff.lastupdate, ff.dateline, ff.foundername, ff.founderuid FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff ON f.fid=ff.fid
	...
	public function fetch_all_for_ranklist($status, $type, $orderfield, $start = 0, $limit = 0, $ignorefids = array()) {
		...
		if($orderfield == 'membernum') {
			$fields = ', ff.membernum';
			$jointable = ' LEFT JOIN '.DB::table('work_forum_forumtestfield').' ff ON ff.fid=f.fid';
	...
	public function delete_by_fid($fids) {
		...
		DB::query("DELETE FROM ".DB::table('work_forum_forumtestfield')." WHERE %i", array(DB::field('fid', $fids)));
	}
	...
	public function fetch_all_validate($start, $limit) {
		return DB::fetch_all("SELECT f.*, ff.dateline, ff.founderuid, ff.foundername, ff.description FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('work_forum_forumtestfield')." ff ON ff.fid=f.fid WHERE status='3' AND level='-1' ORDER BY f.fid DESC LIMIT ".intval($start).', '.intval($limit));
	}
	public function update_archive($fids) {
		return DB::update('forum_forumtest', array('archive' => '0'), "fid NOT IN (".dimplode($fids).")");
	}
	public function fetch_all_for_grouplist($orderby = 'displayorder', $fieldarray = array(), $num = 1, $fids = array(), $sort = 0, $getcount = 0) {
		...
		return DB::fetch_all("SELECT $fieldsql FROM ".DB::table($this->_table)." f $useindex LEFT JOIN ".DB::table("work_forum_forumtestfield")." ff ON ff.fid=f.fid WHERE".($fids ? " $fids AND " : '')." f.type='sub' AND f.status=3 $levelsql $orderby $limitsql");
	}
	...
	function get_forum_by_fid($fid, $field = '', $table = 'forumtest') {
		static $forumlist = array('forumtest' => array(), 'forumtestfield' => array());
		$table = $table != 'forumtest' ? 'forumtestfield' : 'forumtest';
		$return = array();
		if(!array_key_exists($fid, $forumlist[$table])) {
			$forumlist[$table][$fid] = DB::fetch_first("SELECT * FROM ".DB::table('work_forum_'.$table)." WHERE fid=%d", array($fid));
