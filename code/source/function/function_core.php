...
//debug
function miduo($printarr,$printtype) {

	global $_G;

		$dateline = $_G['timestamp'];
		$datetime = date("Y-m-d-H-i-s", $dateline); 

		if(is_array($printarr) || $printtype == "2") {

			file_put_contents("./debug/var_$datetime.txt", var_export($printarr, true)); 
			showmessage("该变量为数组，已帮您打印到【./debug/var_$datetime.txt】文件中!<br /><a href=./debug/var_$datetime.txt target=_blank>请点击前往查看>></a>");

		} elseif($printtype == "noarr") {//适合调试纯字符串

			showmessage($printarr);

		} elseif($printtype == "noshow"){//静默方式，适合调试ajax，查看debug前请先清空./debug/目录下的debug文件

			file_put_contents("./debug/var_$datetime.txt", var_export($printarr, true)); 
			
		} elseif($printtype == "noexit") {//循环模式，适合调试不需要停止的数组
			echo "<br />";
			echo "Debug出以下数据：";
			echo "<br />";
			echo "<pre>";
...
function check_weixin_user_status() {
...
function setnewuser($newusername, $groupid, $password, $email){
	...
	if(empty($password)) {
		...
		$password = $_G['timestamp'].'password';
	}
	
	if(empty($email)) {

		$email = $_G['timestamp'].'@dagaoping.com';
	}	
	
	$newuid = uc_user_register(addslashes($newusername), $password, $email);

	$init_arr = explode(',', $_G['setting']['initcredits']);
	$newadminid = in_array($group['radminid'], array(1, 2, 3)) ? $group['radminid'] : ($group['type'] == 'special' ? -1 : 0);
	C::t('common_member')->insert($newuid, $newusername, $password, $email, 'Manual Acting', $groupid, $init_arr, $newadminid);
...
function browinfo(){
		...
		if(strpos($_SERVER['HTTP_USER_AGENT'], "MSIE 8.0")) {
			...
			$visitor_browser = "iPhone";

		} elseif(strpos($_SERVER['HTTP_USER_AGENT'], "Android")) {

			$visitor_browser = "Android";
